...
# Create subscription page directory
print("Creating subscription page...")
out, err = run("mkdir -p /home/mayowae/public_html/alphaweb/src/app/dashboard/\\(pages\\)/subscription")
print(out, err)
...
const SubscriptionPage = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4 font-inter">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
            <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {[
                { label: 'Status', value: merchant.subscription_status, color: merchant.subscription_status === 'Active' ? 'text-green-600' : 'text-red-600' },
                { label: 'Current Plan', value: merchant.plan ? `${merchant.plan.name} (₦${parseFloat(merchant.plan.pricing).toLocaleString()})` : 'Starter Pack' },
                { label: 'Billing Cycle', value: 'Monthly (30 days)' },
                { label: 'Next Billing Date', value: formatDate(merchant.next_billing_date) },
                { label: 'Trial Ends', value: formatDate(merchant.trial_end_date) },
                { label: 'Outstanding Debt', value: formatNaira(merchant.total_debt) },
              ].map(item => (
...
with sftp.open('/home/mayowae/public_html/alphaweb/src/app/dashboard/(pages)/subscription/page.tsx', 'w') as f:
    f.write(page_content)
sftp.close()
print("Subscription page created!")
