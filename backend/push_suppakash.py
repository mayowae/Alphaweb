...
BASE = '/home/mayowae/suppakash.com'
...
out, err = run("git config --global --add safe.directory /home/mayowae/suppakash.com")
print("OUT:", out)
...
print("\nStep 4: Dump MySQL database (mayowae_wp89) with gzip...")
db_dump_cmd = f"mysqldump -u mayowae_wp89 -p\"[]y32SpT0b\" mayowae_wp89 | gzip > {BASE}/backups/suppakash_wp.sql.gz"
out, err = run(db_dump_cmd)
print("OUT:", out)
if err.strip(): print("ERR:", err)

print("\nStep 5: Package mail directory (suppakash.com)...")
mail_zip_cmd = f"tar -czf {BASE}/backups/suppakash_mail.tar.gz -C /home/mayowae/mail suppakash.com"
out, err = run(mail_zip_cmd)
print("Tar OUT:", out)
if err.strip(): print("Tar ERR:", err)

print("\nStep 6: Write .gitignore and README.md...")
gitignore_content = """/SuppakashBak.zip
/error_log
"""
run(f"cat << 'EOF' > {BASE}/.gitignore\n{gitignore_content}EOF")

readme_content = """# Suppakash Website Backup

This repository contains the files, database, and email backups for `suppakash.com`.

## Restoring Mails
To restore the mail directory:
1. Extract the mailbox archive to your mail directory:
   ```bash
   tar -xzf backups/suppakash_mail.tar.gz -C /home/mayowae/mail/
   ```

## Restoring Database
The database dump is located at `backups/suppakash_wp.sql.gz`. To restore it:
1. Unzip the SQL dump:
   ```bash
   gunzip -c backups/suppakash_wp.sql.gz > backups/suppakash_wp.sql
   ```
2. Import the SQL file:
   ```bash
   mysql -u [db_user] -p [db_name] < backups/suppakash_wp.sql
   ```
"""
...
out, err = run(f"cd {BASE} && git remote add origin git@github.com:charlly-code/suppakash.git")
...
# Git commit
out, err = run(f"cd {BASE} && git commit -m 'first commit - suppakash.com site files, database and mail backups'")
