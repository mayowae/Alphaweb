...
BASE = '/home/mayowae/thepeopleimpact.com'
...
out, err = run("git config --global --add safe.directory /home/mayowae/thepeopleimpact.com")
print("OUT:", out)
...
print("\nStep 4: Package mail directory (thepeopleimpact.com)...")
mail_zip_cmd = f"tar -czf {BASE}/backups/thepeopleimpact_mail.tar.gz -C /home/mayowae/mail thepeopleimpact.com"
out, err = run(mail_zip_cmd)
print("Tar OUT:", out)
if err.strip(): print("Tar ERR:", err)

print("\nStep 5: Write .gitignore and README.md...")
gitignore_content = """/error_log
"""
run(f"cat << 'EOF' > {BASE}/.gitignore\n{gitignore_content}EOF")

readme_content = """# The People Impact Website Backup

This repository contains the landing page files and email backups for `thepeopleimpact.com`.

## Restoring Mails
To restore the mail directory:
1. Extract the mailbox archive to your mail directory:
   ```bash
   tar -xzf backups/thepeopleimpact_mail.tar.gz -C /home/mayowae/mail/
   ```
"""
...
out, err = run(f"cd {BASE} && git remote add origin git@github.com:charlly-code/thepeopleimpact.git")
...
# Git commit
out, err = run(f"cd {BASE} && git commit -m 'first commit - thepeopleimpact.com site files and mail backups'")
