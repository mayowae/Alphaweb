...
interface StaffTableProps {
  ...
  onAddStaff?: (data: StaffFormData) => void;     // ← Add ?
  onUpdateStaff?: (data: StaffFormData, id: string) => void; // ← Add ?
}

const StaffTable: React.FC<StaffTableProps> = ({ 
  staffData, 
  onOpenEditModal,
  onAddStaff,        // ← Added (unused, but required by props)
  onUpdateStaff      // ← Added (unused, but required by props)
}) => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (selectedRoles.length > 0) {
      result = result.filter(item => selectedRoles.includes(item.role));
    }
              ...
              <tbody className="border-b border-[#D9D4D4] w-full">
                {paginatedData.map((item) => (
                  <tr key={item.id} className="bg-white dark:bg-gray-900 transition-all border-b duration-500 hover:bg-gray-50">
                    ...
                    <td className="px-5 py-4 text-gray-600 text-[14px] leading-[20px] font-lato font-normal dark:text-white">{item.fullName}</td>
                    ...
                    <td className="px-5 py-4 text-gray-600 text-[14px] leading-[20px] font-lato font-normal dark:text-white">{item.role}</td>
