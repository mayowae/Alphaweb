...
const getSubscriptionInfo = async (req, res) => {
  try {
    ...
    const merchant = await Merchant.findByPk(merchantId, {
      attributes: [
        'id', 'businessName', 'email', 'subscription_status', 'plan_id', 
        'next_billing_date', 'total_debt', 'trial_end_date', 'is_custom_fee', 'custom_fee'
      ],
      include: [
