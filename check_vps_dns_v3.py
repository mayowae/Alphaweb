...
if __name__ == "__main__":
    ...
    output = run_remote(hostname, port, username, password, [
        "ls -la /var/named/",
        "grep -r 'alphakolect.com' /var/named/ || grep -r 'alphakolect.com' /etc/named.conf",
        "ls -la /etc/nginx/ssl/",
        "pm2 list"
    ])
    with open("vps_dns_check_v3.txt", "w", encoding="utf-8") as f:
        f.write(output)
    print("Saved to vps_dns_check_v3.txt")
