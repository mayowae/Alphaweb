...
const Page = () => {
  ...
  const handleEdit = (application: InvestmentApplication) => {
  ...
  const activeInvestments = applications.filter(app => app.status === 'Approved').length;
  const totalAmount = applications.reduce((sum, app) => sum + Number(app.targetAmount || 0), 0);
    ...
    <div className='w-full'>
      <div className='flex flex-wrap justify-between gap-4 md:gap-0 max-md:flex-col max-md:gap-[10px]'>
        <div className='flex flex-col gap-[3px] min-w-0 w-full md:w-auto'>
          <h1 className='font-inter font-semibold leading-[32px] text-[20px] md:text-[24px]'>Investment Applications</h1>
