...
export default function CustomerDetailsPage() {
  ...
  const handleWithdraw = async () => {
    ...
    setLoading(true)
    try {
      await createWalletTransaction({ type: 'debit', amount: Number(withdrawAmount), description: `Withdrawal to ${withdrawAccountName || 'N/A'} (${withdrawBank || 'N/A'} - ${withdrawAccountNumber || 'N/A'}) for customer ${customerId}` })
      await loadAll()
    } finally {
      setLoading(false)
    }
      ...
      <div className={`fixed inset-0 z-40 ${showWithdrawSidebar ? '' : 'pointer-events-none'}`}>
        ...
        >
          ...
          <div className="p-5">
            ...
            <input value={withdrawBank} onChange={e=>setWithdrawBank(e.target.value)} placeholder="e.g. First Bank" className="w-full border rounded px-3 py-2 mt-1"/>
            <label className="text-xs text-gray-700 mt-4 block">Account Number</label>
            <input value={withdrawAccountNumber} onChange={e=>setWithdrawAccountNumber(e.target.value)} placeholder="e.g. 0123456789" className="w-full border rounded px-3 py-2 mt-1"/>
            <label className="text-xs text-gray-700 mt-4 block">Name</label>
            <input value={withdrawAccountName} onChange={e=>setWithdrawAccountName(e.target.value)} placeholder="Account name" className="w-full border rounded px-3 py-2 mt-1"/>
          ...
          <div className="p-5 border-t">
            <button onClick={()=>{ setShowWithdrawPreview(true) }} disabled={loading || !withdrawAmount} className="w-full bg-indigo-600 text-white rounded px-3 py-2 text-sm">Preview</button>
          </div>
        </aside>
      </div>

      {/* Modal: Withdraw preview */}
      {showWithdrawPreview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="absolute inset-0 bg-black opacity-40" onClick={()=>setShowWithdrawPreview(false)} />
          <div className="relative bg-white rounded-2xl shadow-xl w-[90%] max-w-lg p-6">
            <p className="text-lg font-semibold">PREVIEW</p>
            <div className="mt-4 divide-y">
              <div className="py-3 flex items-center justify-between">
                <span className="text-sm text-gray-600">Bank</span>
                <span className="text-sm font-medium">{withdrawBank || '—'}</span>
              </div>
              <div className="py-3 flex items-center justify-between">
                <span className="text-sm text-gray-600">Account Number</span>
                <span className="text-sm font-medium">{withdrawAccountNumber || '—'}</span>
              </div>
              <div className="py-3 flex items-center justify-between">
                <span className="text-sm text-gray-600">Name</span>
                <span className="text-sm font-medium">{withdrawAccountName || customer?.fullName || '—'}</span>
              </div>
              <div className="py-3 flex items-center justify-between">
                <span className="text-sm text-gray-600">Amount</span>
                <span className="text-sm font-medium">NGN {Number(withdrawAmount||0).toLocaleString()}</span>
              </div>
            </div>
            <div className="mt-5">
              <button
