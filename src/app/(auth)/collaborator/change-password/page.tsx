...
export default function ResetPassword() {
  ...
  const handleContinue = async () => {
    ...
    try {
      // You may want to get the email from context or route, here is a placeholder:
      const email = localStorage.getItem('collaborator_reset_email') || '';
      if (!email) {
        Swal.fire({
          icon: "error",
          title: "Missing Email",
          text: "No email found for password reset. Please restart the process."
        });
        return;
      }
      const result = await collaboratorUpdatePassword(email, newPassword);
      Swal.fire({
        icon: "success",
  ...
  // updatePassword is now handled by handleContinue
          ...
          <div className="bg-white p-8 rounded-lg shadow-xl">
            ...
            <div className="mt-6">
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleContinue(); }} className="mt-8">
                ...
                <div className="mt-8">
                  <button type="button" onClick={handleContinue}  className="w-full py-3 px-4 bg-[#4E37FB] text-white font-bold rounded-lg hover:bg-blue-700 transition duration-200 shadow-md">
                    Continue
                  </button>
                </div>
              </form>
